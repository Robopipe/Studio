...
export class ConfigLoader {
  ...
  public static async getConfig() {
    ...
    const databaseUrl =
      ...
      "postgres://postgres:postgres@postgres:5432/robopipe";
    ...
    const mlBatchServiceAccount =
      process.env.ML_BATCH_SERVICE_ACCOUNT || undefined;
    ...
    const mlBatchGpuCount = process.env.ML_BATCH_GPU_COUNT
      ...
      : undefined;
    const mlBatchBootDiskImage =
      process.env.ML_BATCH_BOOT_DISK_IMAGE || undefined;
    ...
    const mlBatchApiKeySecret =
      process.env.ML_BATCH_API_KEY_SECRET || undefined;
    const mlBatchHubaiApiKeySecret =
      process.env.ML_BATCH_HUBAI_API_KEY_SECRET || undefined;
    ...
    // Gemini (Vertex AI) — hyperparameter suggestions. Auth is ADC (Cloud Run
    // service account / local `gcloud auth application-default login`), so the
    // feature is enabled by gcpProject being set; these only override defaults.
    const geminiModel = process.env.GEMINI_MODEL || undefined;
    const geminiLocation = process.env.GEMINI_LOCATION || undefined;
