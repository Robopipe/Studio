...
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/modules/shadcn/ui/tooltip";
import { ClassificationHistoryEvent, PolygonHistoryEvent, RectangleHistoryEvent } from "@repo/schema";
...
type TimelineEvent = (RectangleHistoryEvent | PolygonHistoryEvent | ClassificationHistoryEvent) & {
  kind: Kind;
  annotationId: number;
};
...
function dedupeEvents(events: TimelineEvent[]): ObjectRow[] {
  ...
  for (const ev of events) {
    ...
    else seen.set(ev.annotationId, { annotationId: ev.annotationId, count: 1, latestAt: ev.createdAt });
  }
...
export const AnnotationHistoryTab = ({
  ...
  const byApiId = useMemo(
    () => new Map(annotations.filter((a) => a.apiId != null).map((a) => [a.apiId!, a])),
    [annotations],
  );
  ...
  const entries: Entry[] = useMemo(() => {
    ...
    for (const g of history.rectangleHistory)
      for (const e of g.events) flat.push({ ...e, kind: "rectangle", annotationId: g.annotationId });
    for (const g of history.polygonHistory)
      for (const e of g.events) flat.push({ ...e, kind: "polygon", annotationId: g.annotationId });
    for (const g of history.classificationHistory)
      for (const e of g.events) flat.push({ ...e, kind: "classification", annotationId: g.annotationId });
  ...
  return (
    <div className="flex flex-col rounded-md border border-border bg-background">
      <div className="flex items-center gap-1.5 px-2 py-1.5 text-[11px] text-muted-foreground">
        <span>{formatDistanceToNow(new Date(entry.latestAt), { addSuffix: true })}</span>
        <span className="text-border">·</span>
        <span className="font-medium text-foreground/80">{entry.editorName}</span>
      </div>
      <div className="flex flex-col border-t border-border">
        {visibleRows.map((row) => {
          const annotation = byApiId.get(row.annotationId);
          const index = annotation ? annotations.indexOf(annotation) : -1;
          const isIsolated = annotation ? isolatedAnnotationId === annotation.id : false;
          return (
            <TimelineObjectRow
...
const TimelineObjectRow = ({ annotation, index, count, latestAt, isIsolated, onIsolate }: TimelineObjectRowProps) => {
  if (!annotation) {
    return (
  ...
  return (
    <Tooltip>
      ...
      >
          <AnnotateIcon className="size-3.5 shrink-0" style={{ color: annotation.color }} />
          ...
          <span className="flex-1 truncate text-[11px] leading-4 text-foreground/90">{annotation.labelName}</span>
          {count > 1 && (
