...
def serve(
    request, path, document_root=None, show_indexes=False, manifest_asset_prefix=None
):
    """
    ...
    if manifest_asset_prefix and not fullpath.exists():
        ...
        manifest_asset_prefix = (
            f"/{manifest_asset_prefix}"
            if not manifest_asset_prefix.startswith("/")
            else manifest_asset_prefix
        )
